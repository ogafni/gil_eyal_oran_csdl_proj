...
class DiscoGAN(object):
    ...
    def get_data(self, number_of_samples=None):
        ...
        elif self.args.task_name == 'celebA_ident':
            data_A, data_B = get_celebA_files(style_A=self.args.style_A, style_B=self.args.style_B, constraint=self.args.constraint,
                                              constraint_type=self.args.constraint_type, test=False, n_test=self.args.n_test)
            test_A, test_B = get_celebA_files(style_A=self.args.style_A, style_B=self.args.style_B, constraint=self.args.constraint,
                                              constraint_type=self.args.constraint_type, test=True, n_test=self.args.n_test)

        elif self.args.task_name == 'celebA_ident_reverse':
            data_A, data_B = get_celebA_files_females(style_A=self.args.style_A, style_B=self.args.style_B, constraint=self.args.constraint,
                                              constraint_type=self.args.constraint_type, test=False, n_test=self.args.n_test)
            test_A, test_B = get_celebA_files_females(style_A=self.args.style_A, style_B=self.args.style_B, constraint=self.args.constraint,
                                              constraint_type=self.args.constraint_type, test=True, n_test=self.args.n_test)

        elif self.args.task_name == 'edges2shoes':
            data_A, data_B = get_edge2photo_files(item='edges2shoes', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_edge2photo_files(item='edges2shoes', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'maps':
            data_A, data_B = get_cycle_files(item='maps', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_cycle_files(item='maps', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'facades':
            data_A, data_B = get_cycle_files(item='facades', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_cycle_files(item='facades', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'cityscapes':
            data_A, data_B = get_cycle_files(item='cityscapes', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_cycle_files(item='cityscapes', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'edges2edges':
            data_A, data_B = get_edge2photo_files(item='edges2shoes', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_edge2photo_files(item='edges2shoes', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'shoes2shoes':
            data_A, data_B = get_edge2photo_files(item='edges2shoes', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_edge2photo_files(item='edges2shoes', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'edges2edges_from_handbags':
            data_A, data_B = get_edge2photo_files(item='edges2handbags', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_edge2photo_files(item='edges2handbags', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'edges2handbags':
            data_A, data_B = get_edge2photo_files(item='edges2handbags', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_edge2photo_files(item='edges2handbags', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'handbags2handbags':
            data_A, data_B = get_edge2photo_files(item='edges2handbags', test=False, number_of_samples=number_of_samples)
            test_A, test_B = get_edge2photo_files(item='edges2handbags', test=True, number_of_samples=number_of_samples)

        elif self.args.task_name == 'handbags2shoes':
            data_A_1, data_A_2 = get_edge2photo_files(item='edges2handbags', test=False, number_of_samples=number_of_samples)
            test_A_1, test_A_2 = get_edge2photo_files(item='edges2handbags', test=True, number_of_samples=number_of_samples)

            data_A = np.hstack([data_A_1, data_A_2])
            test_A = np.hstack([test_A_1, test_A_2])

            data_B_1, data_B_2 = get_edge2photo_files(item='edges2shoes', test=False, number_of_samples=number_of_samples)
            test_B_1, test_B_2 = get_edge2photo_files(item='edges2shoes', test=True, number_of_samples=number_of_samples)
    ...
    def get_test_images(self):
        ...
        if self.args.task_name.startswith('edges2edges'):
            self.test_A = read_images(self.test_style_A, 'A', self.args.image_size)
            self.test_B = read_images(self.test_style_B, 'A', self.args.image_size)
        elif self.args.task_name.startswith('shoes2shoes'):
        ...
        elif self.args.task_name.startswith('handbags2handbags'):
            self.test_A = read_images(self.test_style_A, 'B', self.args.image_size)
            self.test_B = read_images(self.test_style_B, 'B', self.args.image_size)
        elif self.args.task_name.startswith('celebA_ident'):
            self.test_A = read_images(self.test_style_A, None, self.args.image_size)
            self.test_B = read_images(self.test_style_A, None, self.args.image_size)
        elif self.args.task_name.startswith('celebA_ident_reverse'):
            self.test_A = read_images(self.test_style_B, None, self.args.image_size)
            self.test_B = read_images(self.test_style_B, None, self.args.image_size)
        elif self.args.task_name.startswith('edges2'):
            self.test_A = read_images(self.test_style_A, 'A', self.args.image_size)
            self.test_B = read_images(self.test_style_B, 'B', self.args.image_size)
        elif self.args.task_name.startswith('maps'):
            self.test_A = read_images(self.test_style_A, 'A', self.args.image_size, split=600)
            self.test_B = read_images(self.test_style_B, 'B', self.args.image_size, split=600)
        elif self.args.task_name.startswith('facades'):
            self.test_A = read_images(self.test_style_A, 'A', self.args.image_size, split=256)
            self.test_B = read_images(self.test_style_B, 'B', self.args.image_size, split=256)
        elif self.args.task_name.startswith('cityscapes'):
            self.test_A = read_images(self.test_style_A, 'A', self.args.image_size, split=256)
            self.test_B = read_images(self.test_style_B, 'B', self.args.image_size, split=256)
    ...
    def get_images(self, A_path, B_path):
        ...
        if self.args.task_name.startswith('edges2edges'):
            A = read_images(A_path, 'A', self.args.image_size)
            B = read_images(B_path, 'A', self.args.image_size)
        elif self.args.task_name.startswith('shoes2shoes'):
            A = read_images(A_path, 'B', self.args.image_size)
            B = read_images(B_path, 'B', self.args.image_size)
        elif self.args.task_name.startswith('handbags2handbags'):
            A = read_images(A_path, 'B', self.args.image_size)
            B = read_images(B_path, 'B', self.args.image_size)
        elif self.args.task_name.startswith('celebA_ident'):
            A = read_images(A_path, None, self.args.image_size)
            B = read_images(A_path, None, self.args.image_size)
        elif self.args.task_name.startswith('celebA_ident_reverse'):
            A = read_images(B_path, None, self.args.image_size)
            B = read_images(B_path, None, self.args.image_size)
        elif self.args.task_name.startswith('edges2'):
            A = read_images(A_path, 'A', self.args.image_size)
            B = read_images(B_path, 'B', self.args.image_size)
        elif self.args.task_name.startswith('maps'):
            A = read_images(A_path, 'A', self.args.image_size, split=600)
            B = read_images(B_path, 'B', self.args.image_size, split=600)
        elif self.args.task_name.startswith('cityscapes'):
            A = read_images(A_path, 'A', self.args.image_size, split=256)
            B = read_images(B_path, 'B', self.args.image_size, split=256)
        elif self.args.task_name.startswith('facades'):
            A = read_images(A_path, 'A', self.args.image_size, split=256)
            B = read_images(B_path, 'B', self.args.image_size, split=256)
        elif self.args.task_name == 'handbags2shoes' or self.args.task_name == 'shoes2handbags':
    ...
    def initialize(self):
        ...
        self.writer_1 = SummaryWriter(os.path.join(self.result_path, "log_1"))
        ...
        for epoch in range(self.args.epoch_size):
            ...
            for i in range(self.n_batches):
                ...
                if self.args.model_arch == 'discogan':
                    ...
                    self.gen_loss = (self.gen_loss_B * 0.1 + self.fm_loss_B * 0.9) + (self.gen_loss_A * 0.1 + self.fm_loss_A * 0.9)
                    self.dis_loss = self.dis_loss_B + self.dis_loss_A
                elif self.args.model_arch == 'gan_rev':
                    self.gen_loss = (self.gen_loss_A * 0.1 + self.fm_loss_A * 0.9)
                    self.dis_loss = self.dis_loss_A
    ...
    def load_and_test(self):
        self.generator_A = torch.load(
                   os.path.join(self.model_path, 'model_gen_A-' + str(self.args.which_epoch_load)))
        self.generator_B = torch.load(
                   os.path.join(self.model_path, 'model_gen_B-' + str(self.args.which_epoch_load)))

        self.discriminator_A = torch.load(
                   os.path.join(self.model_path, 'model_dis_A-' + str(self.args.which_epoch_load)))
        self.discriminator_B = torch.load(
                   os.path.join(self.model_path, 'model_dis_B-' + str(self.args.which_epoch_load)))

        self.A_path, self.B_path = shuffle_data(self.test_style_A, self.test_style_B)
        ...
        gan_loss_A = (self.gen_loss_B * 0.1 + self.fm_loss_B * 0.9)
        gan_loss_B = (self.gen_loss_A * 0.1 + self.fm_loss_A * 0.9)
    ...
    def get_reg_info(self, generator):

        weights, biases = [], []
        all_params_1 = generator.named_parameters()
        for name, p in all_params_1:
            if 'bias' in name:
                biases += [p]
            else:
                weights += [p]
        ...
        l1_reg_sum = 0
        l2_reg_sum = 0
        l2_reg_sum_squared = 0
        for param in weights:
            l1_reg_sum += torch.norm(param, p=1)
            l2_reg_sum += torch.norm(param, p=2)
            l2_reg_sum_squared += torch.norm(param, p=2) * torch.norm(param, p=2)
    ...
    def finish_iteration(self):
        ...
        if self.iters % self.args.log_interval == 0:
            ...
            all_params_2 = self.generator_B.named_parameters()
            for name, p in all_params_1:
                if 'bias' in name:
                    biases += [p]
                else:
            ...
            self.writer_1.add_scalar('l1_reg_sum', self.as_np(l1_reg_sum), self.iters)
            self.writer_1.add_scalar('l2_reg_sum', self.as_np(l2_reg_sum), self.iters)
            self.writer_1.add_scalar('l2_reg_sum_squared', self.as_np(l2_reg_sum_squared), self.iters)
