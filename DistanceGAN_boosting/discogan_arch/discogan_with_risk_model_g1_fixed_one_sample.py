...
class Disco_with_riskGAN(DiscoGAN):
    ...
    def initialize(self):
        ...
        self.generator_A_2 = Generator(num_layers=self.args.num_layers_second_gan)
        self.generator_B_2 = Generator(num_layers=self.args.num_layers_second_gan)
    ...
    def get_the_images(self, data_style_A, data_style_B, i, with_path=False):
    ...
    def calculate_losses(self, A, B, one_sample_A, one_sample_B, one_sample_index):
        ...
        recon_loss_A_2 += one_sample_recon_loss_A_2

        gen_loss_A_2 += one_sample_gen_loss_A_2
        fm_loss_A_2 += one_sample_fm_loss_A_2
        recon_loss_B_2 += one_sample_recon_loss_B_2

        gen_loss_A_1_total = (gen_loss_B_1 * 0.1 + fm_loss_B_1 * 0.9) * (1. - rate) + recon_loss_A_1 * rate #+ correlation_loss_AB_1 * correlation_rate
        gen_loss_B_1_total = (gen_loss_A_1 * 0.1 + fm_loss_A_1 * 0.9) * (1. - rate) + recon_loss_B_1 * rate #+ correlation_loss_BA_1 * correlation_rate
        ...
        return (gen_loss_1, dis_loss_1, gen_loss_2, dis_loss_2), \
               ...
               recon_loss_A_1, recon_loss_B_1, recon_loss_A_2, recon_loss_B_2,
    ...
    def run(self):
        ...
        if not self.args.test_mode:
            self.generator_A_1 = torch.load(
                    os.path.join(self.args.pretrained_g1_path_A, 'model_gen_A-' + str(self.args.which_epoch_load)))
            self.generator_B_1 = torch.load(
                    os.path.join(self.args.pretrained_g1_path_B, 'model_gen_B-' + str(self.args.which_epoch_load)))
